...
@section('content')
    ...
    <div class="card-body">
        ...
        <a href="{{ url('/presence') }}" title="Back"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left"
                    aria-hidden="true"></i> Back</button></a>
        <a href="{{ url('/presence/' . $presence->id . '/edit') }}" title="Edit Presence"><button
                class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>

        <form method="POST" action="{{ url('/presence' . '/' . $presence->id) }}" accept-charset="UTF-8"
